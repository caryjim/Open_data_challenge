...
```{r}
t2 %>% group_by(ST) %>%
  summarise(Title.I.Status = n_distinct(Title.I.School.Status))
```
...
fct_recode(t2, S = "small", M = "medium", L = "large")
...
head(t2)
```
```{r Drop a Column, Spread the dataframe}
t2 <- t2[ ,c(1:2,4)]
```

```{r}
#Spread
t2 %>% spread(Title.I.School.Status, Title1_percent)
